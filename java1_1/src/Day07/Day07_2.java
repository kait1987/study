...
String[][] 회원목록 = new String[100][2];
		...
		while(true) { // while 문 시작
			...
			if( 선택==1) {
					...
					for (int i = 0 ; i<회원목록.length ; i++) {
						...
						if(회원목록[i][0] !=null) {
							...
							if(회원목록[i][0].equals(아이디) ) {
								...
								System.out.println("--> 이미존재하는 아이디 입니다ㅏ.");
								스위치 = 1;
								break;
								}
				...
				System.out.println("패스워드:"); String 비밀번호 = scan.next();
			...
			if( 선택==2) { // 로그인 : 입력값과 기본값과 비교
				
				System.out.println("--->로그인 진행");
				System.out.println("로그인아이디:"); String 아이디 = scan.next();
				System.out.println("로그인패스워드:"); String 비밀번호 = scan.next();
				
				// 입력값을 배열에 비교 ( 컴퓨터는 순차적으로 찾아서 검사)
				for (int i = 0 ; i<회원목록.length; i++) { // 
																		...
																		if (회원목록[i][0].equals(아이디)  && 회원목록[i][1].equals(비밀번호)) {
																			...
																			while(true) {
																				...
																				for (int j = 0 ; j<회원목록.length; j++) {
																					if( 회원목록[j][0] != null) {
																						System.out.println(회원목록[j][0]+"ㅁ\t\t"+회원목록[j][1]);
																						...
																						if( 선택2==1) {
																							...
																							System.out.println("---> 회원탈퇴 성공");
																								...
																								for(int h = i ; h<99 ; h++) {
																									...
																									if (회원목록[h+1][0] !=null){ // 다른 회원이 null 이면
																										회원목록[h][0] = 회원목록[h+1][0]; //뒤 회원을 한칸 앞으로 이동
																										회원목록[h][1] = 회원목록[h+1][0]; //뒤 회원을 한칸 앞으로 이동
																										}else {
